...
class AnchorHeadTemplate(nn.Module):
    def __init__(self, model_cfg, num_class, class_names, grid_size, point_cloud_range, predict_boxes_when_training):
        ...
        self.do_feature_imitation = getattr(model_cfg, 'do_feature_imitation', False)
    ...
    def assign_targets(self, gt_boxes, data_dict=None):
        """
        Args:
            gt_boxes: (B, M, 8)
        Returns:
        ...
        targets_dict = self.target_assigner.assign_targets(
            self.anchors, gt_boxes, data_dict=data_dict
        )
        return targets_dict
    ...
    def get_cls_layer_loss(self):
        ...
        if 'cam_centerness' in self.forward_ret_dict:
            if True:
                cls_weights = (negative_cls_weights + 1.0 * positives).float().detach()
                ...
                pos_normalizer = positives.sum(1, keepdim=True).float().detach()
                ...
                clone_one_hot_targets = one_hot_targets[..., 1:]
                
            cls_weights = (negative_cls_weights + 1.0 * positives).float()
            cam_centerness = self.forward_ret_dict['cam_centerness']
            pos_normalizer = cam_centerness.sum(1, keepdim=True).float()
            if self.reduce_avg_factor:
                pos_normalizer = dist_reduce_mean(pos_normalizer.view(-1).mean())

            cam_centerness_labels = self.forward_ret_dict['cam_centerness_labels']
            cls_weights /= (pos_normalizer + self.clamp_value)
            cls_targets = cam_centerness_labels * cared.type_as(box_cls_labels)
            cls_targets = cls_targets.unsqueeze(dim=-1)

            cls_targets = cls_targets.squeeze(dim=-1)
            one_hot_targets = torch.zeros(
                *list(cls_targets.shape), self.num_class + 1, dtype=torch.float32, device=cls_targets.device
            )
            one_hot_targets.scatter_(-1, cls_targets.unsqueeze(dim=-1).long(), cam_centerness[..., None].float())

            cls_preds = cls_preds.view(batch_size, -1, self.num_class)
            one_hot_targets = one_hot_targets[..., 1:]
            print(f'{clone_one_hot_targets.sum(dim=1)} -> {one_hot_targets.sum(dim=1)}')
