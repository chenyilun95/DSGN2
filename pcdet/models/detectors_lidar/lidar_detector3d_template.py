...
from pcdet.models import backbones_2d, dense_heads, backbones_3d_lidar
...
class Detector3DTemplate(nn.Module):
    ...
    def build_networks(self):
        model_info_dict = {
            ...
            'boxes_gt_in_cam2_view': getattr(self.dataset, 'boxes_gt_in_cam2_view', None)
        }
    ...
    def build_backbone_3d(self, model_info_dict):
        ...
        backbone_3d_module = backbones_3d_lidar.__all__[self.model_cfg.BACKBONE_3D.NAME](
    ...
    def load_params_from_file(self, filename, logger, to_cpu=False):
        if not os.path.isfile(filename):
            print(f'-------------- ==> No models {filename} are found.')
            return 

        logger.info('==> (LiDAR) Loading parameters from checkpoint %s to %s' % (filename, 'CPU' if to_cpu else 'GPU'))
        ...
        for key in invalid_states:
            logger.info('Not Loaded weight %s: %s' % (key, str(model_state_disk[key].shape)))
