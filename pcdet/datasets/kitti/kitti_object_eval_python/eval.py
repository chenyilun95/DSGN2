...
overlap_0_7 = np.array([[0.7, 0.5, 0.5, 0.7, 0.5, 0.7,        0.7, 0.5, 0.5, 0.5, 0.7, 0.7], 
                        [0.7, 0.5, 0.5, 0.7, 0.5, 0.7,        0.7, 0.5, 0.5, 0.5, 0.7, 0.7],
                        [0.7, 0.5, 0.5, 0.7, 0.5, 0.7,        0.7, 0.5, 0.5, 0.5, 0.7, 0.7]])
overlap_0_5 = np.array([[0.7, 0.5, 0.5,   0.7,  0.5, 0.5,     0.7,  0.5,  0.5,  0.5, 0.7, 0.7], 
                        [0.5, 0.25, 0.25, 0.5, 0.25, 0.5,     0.5, 0.25, 0.25, 0.25, 0.5, 0.5],
                        [0.5, 0.25, 0.25, 0.5, 0.25, 0.5,     0.5, 0.25, 0.25, 0.25, 0.5, 0.5]])

CLASS_NAMES = ['car', 'pedestrian', 'cyclist', 'van', 'person_sitting', 'truck', 
        'car', 'person', 'bicycle', 'motorcycle', 'trailer', 'truck']
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            if i == 0:
                ...
                ret_dict['%s_3d/easy_R40_%s' % (class_to_name[curcls], min_overlaps[i, 0, j])] = mAP3d_R40[j, 0, 1]
                ret_dict['%s_3d/moderate_R40_%s' % (class_to_name[curcls], min_overlaps[i, 1, j])] = mAP3d_R40[j, 1, 1]
                ret_dict['%s_3d/hard_R40_%s' % (class_to_name[curcls], min_overlaps[i, 2, j])] = mAP3d_R40[j, 2, 1]
                ret_dict['%s_bev/easy_R40_%s' % (class_to_name[curcls], min_overlaps[i, 0, j])] = mAPbev_R40[j, 0, 1]
                ret_dict['%s_bev/moderate_R40_%s' % (class_to_name[curcls], min_overlaps[i, 1, j])] = mAPbev_R40[j, 1, 1]
                ret_dict['%s_bev/hard_R40_%s' % (class_to_name[curcls], min_overlaps[i, 2, j])] = mAPbev_R40[j, 2, 1]
                ret_dict['%s_image/easy_R40_%s' % (class_to_name[curcls], min_overlaps[i, 0, j])] = mAPbbox_R40[j, 0, 1]
                ret_dict['%s_image/moderate_R40_%s' % (class_to_name[curcls], min_overlaps[i, 1, j])] = mAPbbox_R40[j, 1, 1]
                ret_dict['%s_image/hard_R40_%s' % (class_to_name[curcls], min_overlaps[i, 2, j])] = mAPbbox_R40[j, 2, 1]

    return result, ret_dict

def get_official_eval_result_2d(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    min_overlaps = np.stack([overlap_0_7, overlap_0_5], axis=0)  # [2, 3, 5]
    name_to_class = {v: n for n, v in class_to_name.items()}
    if not isinstance(current_classes, (list, tuple)):
        current_classes = [current_classes]
    ...
    for anno in dt_annos:
        if anno['alpha'].shape[0] != 0:
            if anno['alpha'][0] != -10:
                compute_aos = True
            break
    mAPbbox, _, _, _, mAPbbox_R40, _, _, _ = do_eval(
        gt_annos, dt_annos, current_classes, min_overlaps, compute_aos, PR_detail_dict=PR_detail_dict)
