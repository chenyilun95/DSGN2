...
from . import augmentor_utils, stereo_database_sampler
...
class StereoDataAugmentor(object):
        ...
        if augmentor_configs is not None:
            ...
            for cur_cfg in aug_config_list:
                ...
                if cur_cfg.NAME == 'random_crop':
                    self.max_crop_shape = (cur_cfg.MAX_CROP_H, cur_cfg.MAX_CROP_W)
    ...
    def gt_sampling(self, config=None):
        db_sampler = stereo_database_sampler.DataBaseSampler(
    ...
    def random_crop(self, data_dict, config=None):
        ...
        data_dict['left_img'] = data_dict['left_img'][y1: y2, x1:x1 + crop_w]
        data_dict['right_img'] = data_dict['right_img'][y1: y2, x1:x1 + crop_w]
        ...
        if 'gt_annots_boxes_2d' in data_dict:
            data_dict['gt_annots_boxes_2d'] = data_dict['gt_annots_boxes_2d'].copy()
            data_dict['gt_annots_boxes_2d'][:, [0, 2]] -= x1
            data_dict['gt_annots_boxes_2d'][:, [1, 3]] -= y1
        return data_dict

    def random_flip(self, data_dict, config=None):
        if np.random.randint(2) > 0.5:
            return data_dict
        
        pts_rect = data_dict['calib'].lidar_pseudo_to_rect(data_dict['points'])
        pts_rect[:, 0] *= -1
        input_points = data_dict['calib'].rect_to_lidar_pseudo(pts_rect)
        data_dict['points'] = input_points
        if 'completion_points' in data_dict:
            completion_pts_rect = data_dict['calib'].lidar_pseudo_to_rect(data_dict['completion_points'])
            completion_pts_rect[:, 0] *= -1
            completion_input_points = data_dict['calib'].rect_to_lidar_pseudo(completion_pts_rect)
            data_dict['completion_points'] = completion_input_points

        data_dict['left_img'], data_dict['right_img'] = data_dict['right_img'][:, ::-1], data_dict['left_img'][:, ::-1]

        # gt boxes
        if 'gt_boxes_2d_ignored' in data_dict:
            data_dict['gt_boxes_2d_ignored'] = box_utils.boxes2d_fliplr(data_dict['gt_boxes_2d_ignored'], data_dict['image_shape'])
        if 'gt_annots_boxes_2d' in data_dict:
            data_dict['gt_annots_boxes_2d'] = box_utils.boxes2d_fliplr(data_dict['gt_annots_boxes_2d'], data_dict['image_shape'])
        
        if 'gt_boxes' in data_dict:
            gt_boxes_camera = box_utils.boxes3d_lidar_to_kitti_camera(
                data_dict['gt_boxes'], data_dict['calib'], pseudo_lidar=True, pseduo_cam2_view=False)
            ...
            gt_boxes_lidar = box_utils.boxes3d_kitti_camera_to_lidar(
                gt_boxes_camera, data_dict['calib'], pseudo_lidar=True, pseudo_cam2_view=False)
            data_dict['gt_boxes'] = gt_boxes_lidar

        data_dict['calib'].fliplr(image_width=data_dict['image_shape'][1])
