...
class StereoDatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None):
        ...
        self.complete_and_predict_depth = getattr(self.dataset_cfg, 'complete_and_predict_depth', False)
        self.voxel_occupancy_range = getattr(self.dataset_cfg, 'voxel_occupancy_range', False)
        self.voxel_occupancy_range_limit = getattr(self.dataset_cfg, 'voxel_occupancy_range_limit', False)

        self.point_cloud_range = np.array(self.dataset_cfg.POINT_CLOUD_RANGE, dtype=np.float32)
        self.stereo_point_cloud_range = np.array(
            getattr(self.dataset_cfg, 'STEREO_POINT_CLOUD_RANGE', self.dataset_cfg.POINT_CLOUD_RANGE), 
            dtype=np.float32)
        ...
        self.max_crop_shape = getattr(self.data_augmentor, 'max_crop_shape', None)
    ...
    def collate_batch(batch_list, _unused=False):
        ...
        for key, val in data_dict.items():
            if key in ['voxels', 'voxel_num_points']:
                ...
                ret[key] = np.stack(padded_imgs, axis=0).transpose(0, 3, 1, 2)
            elif key in ['gt_boxes', 'gt_boxes_no3daug', 'gt_boxes_2d', 'gt_centers_2d', 'gt_annots_boxes_2d', 'gt_boxes_2d_ignored', 'gt_boxes_camera']:
