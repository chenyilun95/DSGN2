...
def range_to_map(yaw, pitch, H=64, W=2048, fov_up=3.0, fov_down=-25.0):
    # laser parameters
...
def range_to_point(yaw, pitch, depth):
    scan_z = np.sin(pitch) * depth
    
    ss = depth ** 2 - scan_z ** 2
    tan_minus_yaw = np.tan(-yaw)
    scan_x = np.sqrt(ss / (1. + tan_minus_yaw ** 2.))
    scan_y = scan_x * tan_minus_yaw

    return np.stack([scan_x, scan_y, scan_z], axis=1)

def torch_point_to_range(points):
    # get depth of all points
...
def torch_range_to_point(yaw, pitch, depth):
    scan_z = torch.sin(pitch) * depth
    
    ss = depth ** 2 - scan_z ** 2
    tan_minus_yaw = torch.tan(-yaw)
    scan_x = torch.sqrt(ss / (1. + tan_minus_yaw ** 2.))
    scan_y = scan_x * tan_minus_yaw

    return torch.stack([scan_x, scan_y, scan_z], axis=1)
...
class LaserScan:
    ...
    def open_scan(self, filename=None, scan=None):
        ...
        remissions = scan[:, 3] if scan.shape[1] == 4 else None  # get remission
    ...
    def do_range_projection(self):
        ...
        # assing to images
        self.proj_range[proj_y, proj_x] = depth # norm depth
